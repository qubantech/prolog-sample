...
Use with DCG:
load.
parse([Какой, продюсер, альбома, Эда_Ширана, ?], Answer).
...
событие('http://ru.dbpedia.org/resource/+_(альбом_Эда_Ширана)', родительный_падеж) --> [альбома, Эда_Ширана].
действие('http://dbpedia.org/ontology/producer', единственное_число, изъявительное_наклонение, настоящее_время, мужской_род) --> [продюсер] .
поиск_личности(Что, Где) --> [Какой], действие(Что, единственное_число, изъявительное_наклонение, настоящее_время, мужской_род), событие(Где, родительный_падеж), ['?'].
